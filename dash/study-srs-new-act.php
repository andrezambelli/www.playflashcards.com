	...
	try {
		...
		if ($user_count_study < $user_max_study) {
			...
			if ($stud_total > 0) {
                ...
                cal_set_session_alert_message('dash.study-srs-new-act.error1');
				
				$redirect = CAL_PATH_WEB . '/dash/deck?k=' . $deck_key;
			}
		} else {
			cal_set_session_error_message('dash.study-srs-new-act.error2');
